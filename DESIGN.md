...
*   **Key Visual Elements**:
    ...
    *   **Role Picker Dropdown**: An interface widget enabling administrative role selection to test access control privileges.
...
*   **Key Visual Elements**:
    ...
    *   **The scrummap_ledger_verifier Output Panel**: A dedicated administrative terminal block displaying live block hashes and verification checks from the SQLite audit database.
...
*   **Key Visual Elements**:
    *   **Side-by-Side Unified Diff View**: Renders the original source code side-by-side with the annotated code blocks. Highlight colors indicate added inline Javadoc comments, metadata tags mapping requirements, or auto-generated skeletal stubs.
...
*   **Key Visual Elements**:
    *   **User Privileges & Role Matrix Table**: Displays the active users, their roles (Product Manager, Scrum Master, Lead Developer, Security Auditor, System Admin), and their authorized endpoints.
    *   **Immutable Transaction Log Viewer**: A paginated console that prints write-ahead ledger blocks directly from the SQLite database. It displays the SHA-256 block hash, the backward-chained signature, transaction timestamps, and client payloads.
    *   **Security Health Score Guardrail Status**: Toggle switches to configure system-wide security properties, such as adjusting the maximum zip archive ceiling (default 1.5GB) and managing the 8-prompt escalation cap.
...
*   **Native Context Retention**: System state is managed directly within the backend's memory space and local SQLite transaction ledger, eliminating dependency on browser storage or heavy database connection pools.
...
sequenceDiagram
    ...
    activate FS
    Note over FS: Run Stage 1: Structural Elimination<br>(Purge node_modules, .git, target, dist - 35% saved)
    Note over FS: Run Stage 2: Syntactic Dilution<br>(Strip comments, white-spaces, and verbose logs)
    ...
    Note over API: Recursively wipe temporary unzipped codebase from drive
    
    API->>DB: Write Transaction Entry (ZIP_CODEBASE_UPLOAD)
    activate DB
    Note over DB: SHA-256 Hashing of current block backward-chained to previous signature
...
To prevent unauthorized log editing or tampering, each transaction entry is linked to its preceding entry using a backward-chained SHA-256 hash algorithm. The block signature $H_n$ is computed mathematically as:

$$H_n = \text{SHA-256}(H_{n-1} \parallel \text{Timestamp} \parallel \text{OperatorID} \parallel \text{TransactionType} \parallel \text{PayloadHash})$$

Where $\parallel$ represents string concatenation. If any past record is modified or a row is deleted, the hash mismatch immediately breaks the chain, raising a high-priority warning during audit runs.
...
erDiagram
    ...
    WRITE_AHEAD_LEDGER {
        ...
        TEXT block_signature "SHA-256 of payload_hash + prev_block_signature"
...
*   **Validation Rules**: Files must end in `.zip`. The compressed file size must not exceed 1.5GB. The file count must not exceed 50,000 files to prevent zip-bomb Denial-of-Service attacks.
*   **Response Payload**:
    ```json
    {
...
*   **Fallback Schema Mode**: If the LLM repeatedly fails to return compliant JSON models (exceeding 3 corrective prompting attempts), the system drops back to a deterministic, local heuristic parser as a fallback, ensuring that the system remains online.
...
*   **Database Transaction Commit Time**: Measures SQLite WAL commit latency, ensuring database writes remain below 5ms.
